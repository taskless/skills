...
The `taskless rules create` command SHALL read a JSON request payload from stdin. The payload SHALL conform to the shape `{ prompt: string, language?: string, successCase?: string, failureCase?: string }`. The `prompt` field is required. If stdin is empty or contains invalid JSON, the CLI SHALL print an error message and exit with a non-zero exit code.

#### Scenario: Valid JSON on stdin

- **WHEN** a user pipes valid JSON with a `prompt` field to `taskless rules create`
- **THEN** the CLI SHALL accept the payload and proceed to submit it to the API

#### Scenario: Missing required fields

- **WHEN** a user pipes JSON missing the `prompt` field
...
- **WHEN** stdin is empty or contains no data
- **THEN** the CLI SHALL print an error indicating a JSON payload is required on stdin
- **AND** the CLI SHALL exit with a non-zero exit code

#### Scenario: Invalid JSON on stdin

- **WHEN** stdin contains data that is not valid JSON
- **THEN** the CLI SHALL print an error indicating the input is not valid JSON
- **AND** the CLI SHALL exit with a non-zero exit code
...
### Requirement: Rules create requires a minimum spec version

The `taskless rules create` command SHALL require a spec version that includes `orgId` and `repositoryUrl` support. If the project's spec version in `.taskless/taskless.json` is older than the minimum required, the CLI SHALL print an error directing the user to update their project config and exit with a non-zero exit code.

#### Scenario: Spec version too old

- **WHEN** `.taskless/taskless.json` has a spec version that predates `orgId`/`repositoryUrl` support
- **THEN** the CLI SHALL print an error indicating the spec version is too old for rule generation
- **AND** the CLI SHALL exit with a non-zero exit code

#### Scenario: Spec version is current

- **WHEN** `.taskless/taskless.json` has a spec version that supports `orgId`/`repositoryUrl`
